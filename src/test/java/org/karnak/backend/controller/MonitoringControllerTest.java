...
class MonitoringControllerTest {
	...
	void getTransfers_returns_empty_page_with_metadata() throws Exception {
		Page<TransferStatusEntity> page = new PageImpl<>(Collections.emptyList(), PageRequest.of(0, 50), 0);
		when(transferMonitoringService.retrieveTransferStatusPageable(any(), any())).thenReturn(page);
	...
	void getTransfers_clamps_excessive_page_size() throws Exception {
		Page<TransferStatusEntity> page = new PageImpl<>(Collections.emptyList(), PageRequest.of(0, 1000), 0);
		when(transferMonitoringService.retrieveTransferStatusPageable(any(), any())).thenReturn(page);
		...
		verify(transferMonitoringService).retrieveTransferStatusPageable(any(), captor.capture());
	...
	void getTransfers_invalid_status_defaults_to_ALL() throws Exception {
		...
		verify(transferMonitoringService).retrieveTransferStatusPageable(captor.capture(), any());
	...
	void getTransfers_forwards_uid_filters() throws Exception {
		...
		mockMvc.perform(get("/api/monitoring/transfers")
			.param("studyUid", "1.2.840")
			.param("serieUid", "1.2.3")
			.param("sopInstanceUid", "1.2.4")
			.param("status", "ERROR")).andExpect(status().isOk());
		...
		org.junit.jupiter.api.Assertions.assertEquals("1.2.4", filter.getSopInstanceUid());
	...
	void countTransfers_returns_count_json() throws Exception {
		when(transferMonitoringService.countTransferStatus(any())).thenReturn(123);
	...
	void exportTransfers_returns_500_on_service_failure() throws Exception {
		...
		// And we never reached the success branch
		verify(transferMonitoringService, never()).retrieveTransferStatus(any());
	}
