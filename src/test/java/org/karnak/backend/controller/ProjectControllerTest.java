...
class ProjectControllerTest {
	...
	void setUp() {
		...
		mockMvc = MockMvcBuilders
			.standaloneSetup(new ProjectController(projectService, secretService, profilePipeService, patientClient))
			.build();
	}
	...
	void createProject_returns_400_for_non_numeric_profile_id() throws Exception {
		mockMvc.perform(post("/api/projects")
			.contentType(MediaType.APPLICATION_JSON)
			.content("{\"name\": \"p\", \"profileId\": \"not-a-number\"}")).andExpect(status().isBadRequest());
	...
	void createProject_returns_404_when_profile_missing() throws Exception {
		...
		mockMvc.perform(post("/api/projects")
			.contentType(MediaType.APPLICATION_JSON)
			.content("{\"name\": \"p\", \"profileId\": 1}")).andExpect(status().isNotFound());
	...
	void updateProject_can_detach_profile_via_null() throws Exception {
		...
		mockMvc.perform(put("/api/projects/1")
			.contentType(MediaType.APPLICATION_JSON)
			.content("{\"profileId\": null}")).andExpect(status().isOk());

		// The detach happens in-place on the same object that was returned by retrieveProject.
	...
	void addSecret_rejects_invalid_hex_length() throws Exception {
		...
		mockMvc.perform(post("/api/projects/1/secrets")
			.contentType(MediaType.APPLICATION_JSON)
	...
	void addExternalId_rejects_invalid_birth_date() throws Exception {
		...
		mockMvc.perform(post("/api/projects/1/external-ids")
			.contentType(MediaType.APPLICATION_JSON)
	...
	void addExternalId_returns_404_when_project_missing() throws Exception {
		...
		mockMvc.perform(post("/api/projects/99/external-ids")
			.contentType(MediaType.APPLICATION_JSON)
			.content("{\"pseudonym\":\"P\",\"patientId\":\"ID\"}")).andExpect(status().isNotFound());
	}
	...
	void addExternalId_returns_409_when_patient_exists() throws Exception {
		...
		mockMvc.perform(post("/api/projects/1/external-ids")
			.contentType(MediaType.APPLICATION_JSON)
			.content("{\"pseudonym\":\"P\",\"patientId\":\"ID\"}")).andExpect(status().isConflict());
	}
	...
	void importExternalIds_skips_invalid_dates_without_500() throws Exception {
		...
		mockMvc.perform(post("/api/projects/1/external-ids/import")
			.contentType(MediaType.APPLICATION_JSON)
			.content(
					"[{\"pseudonym\":\"A\",\"patientId\":\"1\"}," + "{\"pseudonym\":\"B\",\"patientId\":\"2\",\"patientBirthDate\":\"junk\"}]"))
			.andExpect(status().isOk())
