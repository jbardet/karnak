...
class AuthConfigControllerTest {
	...
	void getAllAuthConfigs_when_present_returns_list_and_never_exposes_clientSecret() throws Exception {
		...
		assertFalse(body.contains("super-secret"),
				"clientSecret must not be serialized in API responses: " + body);
	}
	...
	void createAuthConfig_rejects_blank_code() throws Exception {
		...
		mockMvc.perform(post("/api/auth-configs")
			.contentType(MediaType.APPLICATION_JSON)
			.content(objectMapper.writeValueAsString(cfg))).andExpect(status().isBadRequest());
	...
	void createAuthConfig_conflict_when_code_exists() throws Exception {
		...
		mockMvc.perform(post("/api/auth-configs")
			.contentType(MediaType.APPLICATION_JSON)
			.content(objectMapper.writeValueAsString(incoming))).andExpect(status().isConflict());
	...
	void updateAuthConfig_only_updates_provided_fields() throws Exception {
		...
		mockMvc.perform(put("/api/auth-configs/k")
			.contentType(MediaType.APPLICATION_JSON)
			.content("{\"scope\": \"new-scope\"}")).andExpect(status().isOk());
